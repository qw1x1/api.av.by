...
async def callbacks_cars(callback:types.CallbackQuery):
    ...
    id_gen[callback.from_user.id] = action
    pars_info = Pars_info_id_file(brand_id=int(Start.brand[callback.from_user.id]), 
                                  model_id=int(Model.id_model[callback.from_user.id]),generations_id=id_gen[callback.from_user.id])
    ...
    if cars_count_page == 0:
        ...
        if len(list_cars) != 0:
            create_request(brand_id=int(Start.brand[callback.from_user.id]), model_id=int(Model.id_model[callback.from_user.id]), user=callback.from_user.id)
            trek = 0
