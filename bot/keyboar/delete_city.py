...
async def new_keyboard(message:types.Message,text:str):
    Start.del_city_bilder[message.from_user.id]=NewKeyboard(Start.del_city_bilder[message.from_user.id],text)
    await message.edit_text("Выберите город который хотите удалить:", reply_markup=Start.del_city_bilder[message.from_user.id].as_markup())
